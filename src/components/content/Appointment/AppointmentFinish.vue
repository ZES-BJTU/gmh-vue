<template>
  <div class="AppointmentFinish">
    ...
    <AProject :visible="projectsDialog"
      v-on:closeDialog="projectsDialog = false"
      v-on:addAProject="handleAddAProject"></AProject>
  </div>
</template>
          ...
          if(this.newProject.projects.length == 0){
            ...
            this.$store
              ...
              .then(res => {
                if (res.code === 0) {
                  this.$store
                    .dispatch("finishAppointment", this.$route.params.id)
                    .then(res => {
                      if (res.code === 0) {
    ...
    loadEmployeeAll(){
      this.$store.dispatch("loadEmployeeAll").then( res => {
        this.$store.dispatch("loadProjectAll").then( res => {
          ...
          this.newProject.customerMobile = Number.parseInt(app.customerMobile);
          for( let appointmentProject of app.appointmentProjects){
            let project = this.$store.getters.getProjectFromAllById(appointmentProject.projectId);
            let operator = this.$store.getters.getEmployeeFromAllById(appointmentProject.employeeId);
            this.projectsCopy.push({
              'id': project.id,
              'name': project.name,
              'amount': 1,
