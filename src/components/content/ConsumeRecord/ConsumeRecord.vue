<template>
  <div class="ConsumeRecord">
    <el-form :inline="true" :model="consumeRecordSearch" ref="consumeRecordSearch" class="demo-form-inline search-form" 
      @keyup.enter.native="searchConsumeRecord('search')">
      <el-form-item>
        <!-- 添加隐藏的input 阻止一个input时的默认回车事件 -->
        <el-input style="display:none;"></el-input>
        <el-input v-model.trim="consumeRecordSearch.content" placeholder=""></el-input>
      </el-form-item>
      <el-form-item>
        <el-button type="primary" @click="searchConsumeRecord('search')" icon="el-icon-search" class="search-btn">查询</el-button>
      </el-form-item>
    </el-form>
    <div class="operate-box">
      <router-link to="/consume-record/new-card">
        <el-button type="primary" icon="el-icon-plus">办卡</el-button>
      </router-link>
      <router-link to="/consume-record/new-product">
...
export default {
  ...
  methods: {
    searchConsumeRecord(type) {
      this.loading = true;
      if(type === 'search'){
        this.consumeRecordSearch.oldContent = this.consumeRecordSearch.content;
      }
      this.consumeRecordSearch.type = type;
      this.$store.dispatch("loadConsumeRecord", this.consumeRecordSearch).then( res => {
        this.loading = false;
    ...
    chagePage(val){
      this.consumeRecordSearch.pageNum = val;
      this.searchConsumeRecord('page');
    },
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            this.$store.dispatch("cancelAppointment", {
              ...
              this.searchConsumeRecord('search');
            }).catch( err => {
              done();
              instance.confirmButtonLoading = false;
              console.log(err);
