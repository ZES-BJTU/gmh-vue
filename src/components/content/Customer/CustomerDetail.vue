...
export default {
    ...
    onSubmit(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          this.$store.dispatch("modCustomer",{
            id: this.modCustomerForm.id,
            name: this.modCustomerForm.name,
            gender: this.modCustomerForm.gender,
            mobile: this.modCustomerForm.mobile,
            //考虑到接口失败的情况，不能直接修改把原来的日期转化成时间戳，否则getDate()失效
            birthday: this.toTimeStamp(this.modCustomerForm.birthday),
            source: this.modCustomerForm.source,
            remark: this.modCustomerForm.remark
          }).then( res => {
            if( res.code === 0){
  ...
  beforeMount: function () {
    if(this.$store.state.customer.customers.length === 0){
      ...
      this.modCustomerForm.birthday = this.toDateObject(customer.birthday);
