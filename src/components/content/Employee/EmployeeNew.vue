      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          this.$store.dispatch("addEmployee", {
            name: this.newEmployeeForm.name,
            gender: this.newEmployeeForm.gender,
            mobile: this.newEmployeeForm.mobile,
            //考虑到接口失败的情况，不能直接修改把原来的日期转化成时间戳，否则getDate()失效
            entryTime: this.toTimeStamp(this.newEmployeeForm.entryTime),
            workTypes: this.newEmployeeForm.workTypes,
            remark: this.newEmployeeForm.remark
          }).then(res => {
              if (res.code === 0) {
