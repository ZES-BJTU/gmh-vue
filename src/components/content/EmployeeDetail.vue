...
export default {
    ...
    onSubmit(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          this.modEmployeeForm.entryTime = this.toTimeStamp(this.modEmployeeForm.entryTime);
          this.$store.dispatch("modEmployee", this.modEmployeeForm).then( res => {
            if( res.code === 0){
  ...
  beforeMount: function () {
    if(this.$store.state.employee.employees.length === 0){
      ...
      this.modCustomerForm.gender = ( employee.gender === '男' ? 1 : 0);
      this.modCustomerForm.mobile = Number.parseInt(employee.mobile);
      this.modCustomerForm.entryTime = employee.entryTime;
      this.modCustomerForm.workTypes = employee.workTypes;
      this.modCustomerForm.remark = employee.remark;
    }
  }
};
</script>
